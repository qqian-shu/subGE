...
for i in range(m):
    if target[i] == 0:
        ...
        y.append(1)
y = pd.DataFrame(y)
# y.to_csv('./bbbp_data./target.csv', index=0, header=0)
...
for i in range(n):
    ...
    adj.to_csv('./bbbp_data./'+str(i)+'_adj.csv', index=0, header=0)
    ...
    feature.to_csv('./bbbp_data./'+str(i)+'_feature.csv', index=0, header=0)
